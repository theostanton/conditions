...
function wrapHtml(title: string, description: string, body: string, noCache: boolean = false): string {
    const cacheTag = noCache
        ? ''
        : '';
    return `<!DOCTYPE html>
<html lang="en">
...
export function renderReportPage(massif: Massif, report: ConditionsReport, bulletin: Bulletin): string {
    ...
    const riskBadgeHtml = riskLevel
        ? `<span class="risk-badge" style="${riskBadgeStyle(riskLevel)}">Avalanche Risk: ${riskLevel}/5 ${riskLabel ? `(${riskLabel})` : ''}</span>`
        : '';
...
export function renderLoadingPage(massif: Massif): string {
    ...
    return wrapHtml(title, description, body, true);
}
